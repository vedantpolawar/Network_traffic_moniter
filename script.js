...
async function processOne() {
  ...
  await highlightStep(1, `Enqueued to ring buf (size=${ring.size}/${cap})`);
  await sleep(80);
...
async function processBatch(n) {
  ...
  addLog("BATCH", `+${n} packets processed (total: ${totalPackets})`, "info");
  renderAll();
}
...
function toggleAuto() {
  if (autoInterval) {
    ...
    autoInterval = setInterval(() => processBatch(1).then(renderAll), simSpeed);
...
function renderAll() {
  ...
  renderTopK();
  renderMemory();
  if (totalPackets > 10) detectAndShowAttack();
}
...
function renderMetrics() {
  ...
  document.getElementById("mLoad").textContent = (lf * 100).toFixed(0) + "%";
}
...
function renderHashTable() {
  ...
  const lf = ht.loadFactor();
  document.getElementById("htLoadTag").textContent = `α = ${lf.toFixed(2)}`;
...
function loadExample(type) {
  const examples = {
    ...
    synflood:
      "203.45.12.7, 91.108.4.3, 5.188.210.22, 141.8.224.10, 62.233.50.1, 45.142.212.5, 176.119.4.12, 185.220.101.8, 23.129.64.100, 104.244.76.5, 198.96.155.3, 209.141.46.2, 37.187.129.7, 51.91.77.4, 167.88.15.9, 193.239.232.1, 80.67.172.11, 188.166.23.4, 46.165.230.5, 94.130.17.3",
    portscan:
      "10.99.0.1, 10.99.0.2, 10.99.0.3, 10.99.0.4, 10.99.0.5, 10.99.0.6, 10.99.0.7, 10.99.0.8, 10.99.0.9, 10.99.0.10, 10.99.0.11, 10.99.0.12, 10.99.0.13, 10.99.0.14, 10.99.0.15, 10.99.0.16, 10.99.0.17, 10.99.0.18, 10.99.0.19, 10.99.0.20",
  };
...
function updateTradeoff() {
  ...
  document.getElementById("tradeBody").innerHTML = bufSizes
    .map((b, i) => {
      ...
      const barColor = acc < 70 ? "#ff4466" : acc < 85 ? "#ffaa00" : "#00ff88";
...
let attackPacketCount = 0;
const ATTACK_TOTAL = 40;
...
const ATTACK_CONFIGS = {
  ddos: {
    ...
    color: "#ff4466",
    description:
      "Volumetric flood from single attacker IP overwhelming bandwidth",
    detect: (ht, ring) => {
      const entries = ht.entries();
      if (!entries.length || ring.size < 4) return false;
      const topCount = Math.max(...entries.map(([, v]) => v));
      return topCount / ring.size > 0.55;
    },
    genIP: () => "203.0.113.99",
  },
  syn: {
    ...
    badge: "SYN Flood",
    color: "#aa66ff",
    description: "Half-open connection flood using spoofed source IPs",
    detect: (ht, ring) => {
      if (ring.size < 6) return false;
      const entries = ht.entries();
      const singlesRatio =
        entries.filter(([, v]) => v === 1).length / Math.max(1, entries.length);
      return singlesRatio > 0.65 && entries.length > 5;
    },
    genIP: () => {
  ...
  portscan: {
    ...
    badge: "Port Scan",
    color: "#00ff88",
    description: "Sequential probe sweep from single reconnaissance source",
    detect: (ht, ring) => {
      if (ring.size < 5) return false;
      const buf = ring.toArray().filter(Boolean);
      if (buf.length < 5) return false;
      const srcCounts = {};
      buf.forEach((ip) => {
        srcCounts[ip] = (srcCounts[ip] || 0) + 1;
      });
      const top = Object.values(srcCounts).sort((a, b) => b - a)[0];
      const scannerRatio = top / buf.length;
      const allSameSubnet = buf.every((ip) => ip && ip.startsWith("10.99."));
      return allSameSubnet && scannerRatio > 0.45;
    },
    genIP: () => `10.99.0.${(attackPacketCount % 20) + 1}`,
  },
};
...
async function injectAttack(type) {
  ...
  addLog("ATTACK", `⚠ ${cfg.label} injection started`, "error");

  attackInterval = setInterval(async () => {
    if (attackPacketCount >= ATTACK_TOTAL) {
      clearInterval(attackInterval);
      attackInterval = null;
      addLog(
        "ATTACK",
        `${cfg.label} injection complete (${ATTACK_TOTAL} pkts)`,
        "warn",
      );
      return;
    }
...
function detectAndShowAttack() {
  for (const [type, cfg] of Object.entries(ATTACK_CONFIGS)) {
    if (cfg.detect(ht, ring)) {
      showAlert(type, cfg);
      return;
    }
  }
...
function showAlert(type, cfg) {
  const el = document.getElementById("attackAlert");
  const title = document.getElementById("attackAlertTitle");
  const desc = document.getElementById("attackAlertDesc");
  const badge = document.getElementById("attackAlertBadge");
  if (el.dataset.showing === type) return;
  el.dataset.showing = type;
  title.textContent = `⚠ ${cfg.label.toUpperCase()} DETECTED`;
  ...
  el.style.boxShadow = `0 0 18px ${cfg.color}44`;
  el.classList.remove("hidden");
  addLog(
    "DETECT",
    `🚨 ${cfg.label} pattern detected in traffic window`,
    "error",
  );
}
